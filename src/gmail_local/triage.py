...
class TriagePolicy:
    ...
    financial_patterns: List[re.Pattern] = field(default_factory=lambda: [
        re.compile(r"\b(account statement|account report|portfolio report|checking balance|rewards summary|statements?|tax form|1099|w-2|irs|tax return|portfolio|dividend|brokerage)\b", re.I),
        re.compile(r"@(.*\.)?(chase|wellsfargo|mywellsfargorewards|fidelity|vanguard|americanexpress|citi|bankofamerica|capitalone|acorns|robinhood|schwab|etrade)\.com", re.I),
    ])
